  ...
  if (done) {
    return (
      <div className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-slate-100 text-center">
        ...
        <p className="mt-2 text-sm text-slate-400">Your password has been changed successfully.</p>
        <button
  ...
  return (
    <div className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-slate-100">
      <div className="mb-7">
        ...
        <p className="mt-1 text-sm text-slate-400">Update your account password. You'll need your current password.</p>
      </div>

      <AlertBanner message={error} />

      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-1">
        <PasswordField label="Current Password" field="old_password" placeholder="Enter current password" formData={formData} errors={errors} updateFormData={updateFormData} rules={RULES.old_password} />
        <PasswordField label="New Password" field="new_password" placeholder="Min. 8 characters" formData={formData} errors={errors} updateFormData={updateFormData} rules={RULES.new_password} />
