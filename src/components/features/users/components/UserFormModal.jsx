...
const UserFormModal = ({ open, onClose, onSubmit, loading, error, fieldErrors, user }) => {
  const isEdit = !!user;
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-6 py-5">
          ...
          <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            ...
            >
              <span className={`absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform duration-200 ${form.is_active ? "translate-x-5" : ""}`} />
            </button>
          </div>
