...
export default function ProjectDetailView() {
  ...
  const handleDelete = async () => {
    try {
      ...
      success(
        ...
        `"${project?.title}" ${t("projects.toast_deleted_sub")}`
      );
  ...
  const statusLabel = t(`projects.status_${project.status}`, {
    defaultValue: project.status,
  });
  const categoryName =
    project.category?.name_ar && i18n.language === "ar"
    ...
    <div className="mx-auto flex max-w-5xl flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6">
              ...
              items={[
                {
                  ...
                  onClick: () => navigate(`${base}/projects/${id}/edit`),
                },
                {
                  label: project.is_published
                    ? t("projects.unpublish")
                    : t("projects.publish"),
                  icon: project.is_published ? (
                    <MdPublicOff className="h-4 w-4" />
                  ) : (
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex items-start justify-between gap-4 px-6 pb-6 pt-4">
          <div className="min-w-0 flex-1">
            ...
            {project.title_ar && (
              <p
                className="mt-0.5 text-base font-semibold text-slate-500"
                dir="rtl"
              >
                {project.title_ar}
              </p>
            ...
            {project.summary_ar && (
              <p className="mt-0.5 text-sm text-slate-400" dir="rtl">
                {project.summary_ar}
              </p>
            )}
      ...
      {(project.target ||
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <FormHeader
            ...
            title={t("projects.funding_impact")}
            subtitle={t("projects.funding_impact_sub")}
          />
          {project.target && (
            <div className="mb-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {project.target && (
              <InfoRow
                icon={<MdAttachMoney className="h-4 w-4" />}
                label={t("projects.target_info")}
                value={fmtMYR(project.target)}
              />
            )}
            {project.amount_raised && (
              <InfoRow
                icon={<MdTrendingUp className="h-4 w-4" />}
                label={t("projects.raised_info")}
                value={fmtMYR(project.amount_raised)}
              />
            )}
            {project.total_beneficiaries_helped && (
              <InfoRow
                icon={<MdPeople className="h-4 w-4" />}
                label={t("projects.beneficiaries_info")}
                value={project.total_beneficiaries_helped}
              />
            )}
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        <FormHeader
          ...
          title={t("projects.project_info")}
          subtitle={t("projects.project_info_sub")}
        />
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <InfoRow
            icon={<MdCalendarToday className="h-4 w-4" />}
            label={t("projects.start_date_info")}
            value={fmtDate(project.start_date)}
          />
          <InfoRow
            icon={<MdCalendarToday className="h-4 w-4" />}
            label={t("projects.end_date_info")}
            value={fmtDate(project.end_date)}
          />
          <InfoRow
            icon={<MdPerson className="h-4 w-4" />}
            label={t("projects.created_by_info")}
            value={project.created_by || "—"}
          />
          <InfoRow
            icon={<MdUpdate className="h-4 w-4" />}
            label={t("projects.last_updated_info")}
            value={fmtDate(project.updated_at)}
          />
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        <div className="mb-4 flex items-center justify-between">
          <FormHeader
            ...
            title={t("projects.assigned_staff")}
            subtitle={t("projects.assigned_staff_sub")}
          />
          <Button
            variant="secondary"
      ...
      {(project.description ||
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <FormHeader
            icon={<MdAssignment className="h-5 w-5" />}
            title={t("projects.description_section")}
            subtitle={t("projects.description_section_sub")}
          />
