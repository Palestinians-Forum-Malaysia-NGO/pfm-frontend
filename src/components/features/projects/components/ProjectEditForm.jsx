...
export default function ProjectEditForm() {
    ...
    fetchProject(id)
      .then((data) => {
        ...
        const snap = {
          ...
          cover_image: null,
          classification_ids: Array.isArray(data.classification_ids)
            ? data.classification_ids
            : (data.classifications ?? []).map(
                (classification) => classification.id
              ),
          category_id:
            categories.find((c) => c.slug === data.category?.slug)?.id ?? "",
  ...
  useEffect(() => {
    if (form.category_id || !project?.category?.slug || categories.length === 0)
      return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      success(
        t("projects.toast_updated"),
        `"${form.title}" ${t("projects.toast_updated_sub")}`
      );
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6">
      <PageHeader
        ...
        actions={
          <Button
            ...
            text={t("projects.back_to_project")}
            onClick={() => navigate(`${base}/projects/${id}`)}
          />
        }
      />
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <FormHeader
            ...
            title={t("projects.cover_section")}
            subtitle={t("projects.cover_subtitle")}
          />
          <StorageCoverField
            folder="projects"
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <FormHeader
            ...
            subtitle={t("projects.overview_subtitle")}
          />

          {/* Title EN / AR */}
          <div className="grid grid-cols-1 gap-x-5 sm:grid-cols-2">
            <InputField
              label={t("projects.title_en")}
              field="title"
              placeholder="e.g. Community Aid Programme 2026"
              formData={form}
              errors={errors}
              updateFormData={set}
              rules={RULES.title}
            />
            <InputField
              label={t("projects.title_ar_label")}
              field="title_ar"
              placeholder={t("projects.title_ar_placeholder")}
              required={false}
              formData={form}
              errors={errors}
              updateFormData={set}
          ...
          {project?.slug && (
            <div className="mb-3 mt-1">
              <p className="mb-1 text-xs font-medium text-slate-400">
                {t("projects.slug_label")}
              </p>
              <p className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 font-mono text-xs text-slate-500">
                {project.slug}
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 gap-x-5 sm:grid-cols-2">
            ...
            />
            <TextareaField
              label={t("projects.summary_ar_label")}
              field="summary_ar"
              rows={2}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <FormHeader
            icon={<MdAssignment className="h-5 w-5" />}
            title={t("projects.audience_section")}
            subtitle={t("projects.audience_subtitle")}
          />
          <ToggleInput
            label={t("projects.featured_toggle")}
            field="is_featured"
            formData={form}
            errors={errors}
            updateFormData={set}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <FormHeader
            icon={<MdAssignment className="h-5 w-5" />}
            title={t("projects.details_section")}
            subtitle={t("projects.details_subtitle")}
          />

          {/* Description EN / AR */}
          <div className="grid grid-cols-1 gap-x-5 sm:grid-cols-2">
            <TextareaField
              label={t("projects.desc_en")}
              field="description"
              rows={5}
            ...
            />
            <TextareaField
              label={t("projects.desc_ar_label")}
          ...
          <div className="grid grid-cols-1 gap-x-5 sm:grid-cols-2">
            <TextareaField
              label={t("projects.beneficiary_en")}
            ...
            />
            <TextareaField
              label={t("projects.beneficiary_ar_label")}
              field="beneficiary_info_ar"
              rows={3}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6">
          <FormHeader
            icon={<MdAssignment className="h-5 w-5" />}
            title={t("projects.funding_section")}
            subtitle={t("projects.funding_subtitle")}
          />
          <InputField
            label={t("projects.target_label")}
            field="target"
            type="number"
          ...
          <ToggleInput
            label={t("projects.publish_toggle_edit")}
            field="is_published"
            formData={form}
            errors={errors}
            updateFormData={set}
        ...
        <div className="flex gap-3">
          ...
          <Button
            type="submit"
            variant="primary"
            text={t("projects.save_btn")}
            loading={saving}
            disabled={!form.title.trim() || !isDirty || saving}
            className="flex-1"
          />
        </div>
      </form>
