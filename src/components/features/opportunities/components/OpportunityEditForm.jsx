...
import { MdArrowBack, MdEdit, MdWork } from "react-icons/md";
...
import { useGetOpportunity, useUpdateOpportunity } from "components/features/opportunities/hooks";
...
export default function OpportunityEditForm() {
  ...
  const { execute: fetchOpportunity, loading, error: loadError } = useGetOpportunity();
  const { execute: updateOpportunity, loading: saving, error: saveError } = useUpdateOpportunity();
  ...
  useEffect(() => {
    fetchOpportunity(id).then((data) => {
      ...
      const snapshot = {
        ...
        available_positions:  data.available_positions != null ? String(data.available_positions) : "1",
        for_public:           data.for_public  ?? true,
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      success(t("opportunities.toast_updated"), `${formData.title} ${t("opportunities.toast_updated_sub")}`);
      navigate(`${base}/opportunities/${id}`);
    } catch (err) {
      toastError(t("opportunities.toast_update_failed"), err?.message);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-5xl flex flex-col rounded-2xl border border-slate-200 bg-white">
      ...
      <PageHeader
        ...
        subtitle={formData.title || t("opportunities.detail_subtitle")}
        className="p-6 border-b border-slate-200"
        actions={
          <Button variant="ghost" icon={<MdArrowBack className="h-4 w-4" />} text={t("opportunities.back_to_opportunity")} onClick={() => navigate(`${base}/opportunities/${id}`)} />
        }
      />
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col">
        ...
        <div className="flex gap-3 p-6">
          <Button variant="ghost" text={t("opportunities.cancel")} onClick={() => navigate(`${base}/opportunities/${id}`)} className="flex-1" />
          <Button
            type="submit" variant="primary" text={t("opportunities.save_changes")}
            loading={saving}
